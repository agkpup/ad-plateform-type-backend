...
const io = require('socket.io')(server,{cors:{origin:'http://192.168.1.19:8080'}});
server.listen(port,()=>{
...
app.get('/auth',(req,res,next)=>{
    ...
    app.post('/auth/login', function(req, res, next) {
      ...
      con.query(`SELECT * from deda_198003.auth WHERE mobile = ${req.body.mobile}`,(err,resu)=>{
        if(err){
          ...
          resu.forEach(element => {
            bcrypt.compare(req.body.password,element.password, function(err,val) {
              ...
              if(val==true){
                ...
                res.json({msg:'Successful login',variant:'success',data:{ID:element.id,Name:element.name,mobile:element.mobile,token:token}})
                var d = new Date();
          ...
          bcrypt.genSalt(saltRounds, (err, salt) => {
            bcrypt.hash(req.body.password, salt, (err, hash) => {
                ...
                con.query(`SELECT * FROM deda_198003.auth WHERE mobile = ${req.body.mobile}`,(err,result)=>{
                  ...
                  if(!!result.length > 0 ){
                    res.json({msg:'Already created account',variant:'info'});
                  }else{
                    con.query('INSERT INTO deda_198003.auth (name,mobile,password) VALUES (?,?,?)',[req.body.name,req.body.mobile,hash],(err,result)=>{
                      if(err){
                        res.json({msg:'Unsuccessfull please try again',variant:'danger'});
                        console.log("err at register auth",err);
                      }else{
                        res.json({msg:'successfully created account',variant:'success'});
...
io.use((socket, next) => {
  ...
  //console.log({'token getting from client':token})
  if (!token || token === '') {
    console.log('No Token provided')
  }else{
  jwt.verify(token,process.env.secret,(e,r)=> {
    if(e){
     throw e;
      ...
      console.log("verified req socket")
    }
  })
next()
}
...
function myContact(params) {
    ...
    function userSend(params) {
      con.query(`SELECT id as userId,name,mobile,verified from auth where id IN (${params})`,(err4,res4)=>{
        if(err4) throw err4;
        if(res4){
 ...
 socket.on("chatHistory",(params)=>{
   ...
   con.query(`SELECT DISTINCT msg_from,msg_to,msgIn,time
   
   from Chat where (msg_from,msg_to) =('${params.msg_from}','${params.msg_to}') OR (msg_to,msg_from) =('${params.msg_from}','${params.msg_to}')`,(err,data)=>{
     if(err) throw err;
     if(!!data){
  ...
  socket.on("getNameById",(data)=>{
    ...
    con.query(`SELECT id,name from auth where id=${data}`,(e,r)=>{
      if(e) throw e;
      console.log({e:e});
      if(r){
